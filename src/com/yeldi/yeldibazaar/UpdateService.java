	...
	protected void onHandleIntent(Intent intent) {
		...
		try {
			...
			if (success) {
				...
				if (!d.exists()) {
					Log.d("FDroid",
							"Icons were wiped. Re-downloading all of them.");
					d.mkdirs();
			...
			if (success && changes && notify && (newUpdates > prevUpdates)) {
				...
				Intent notifyIntent = new Intent(this, FDroid.class).putExtra(
						FDroid.EXTRA_TAB_UPDATE, true);
				if (newUpdates > 1) {
